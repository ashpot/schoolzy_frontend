...
interface Props { groups: ClassGroup[]; onDelete: (id: string) => void; }

const PAGE_SIZE = 8;

export default function ClassGroupsTable({ groups, /*onDelete*/ }: Props) {
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50/80 border-b border-border-line02">
              {["#", "Name", "Code", "Parent Class", "Actions"].map((h) => (
          ...
          <motion.tbody variants={staggerContainer} initial="hidden" animate="show" key={page}>
            {paginated.map((g, i) => (
              <motion.tr key={g.id} variants={rowVariant} className="border-b border-border-line02 hover:bg-gray-50/50">
                ...
                <td className="py-3.5 px-4">
                  <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border ${getClassBadgeColor(g.parentClass)}`}>
                    {g.parentClass}
                  </span>
                </td>
                <td className="py-3.5 px-4" onClick={(e) => e.stopPropagation()}>
                  {/* <DeleteButton onDelete={() => deleteGroup.mutate(g.id, { onSuccess: () => onDelete(g.id) })} /> */}
                </td>
              </motion.tr>
            ))}
          </motion.tbody>
