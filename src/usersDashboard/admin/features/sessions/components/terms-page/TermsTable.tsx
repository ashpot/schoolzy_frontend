...
export default function TermsTable({ terms, onDelete }: Props) {
  ...
  const filtered = terms.filter(
    (t) =>
      ...
      t.sessionName.toLowerCase().includes(search.toLowerCase())
  );
    ...
    <div className="bg-white rounded-2xl card-shadow overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-border-line02">
        <div className="flex items-center gap-2">
          ...
          <h3 className="section-title">Terms List</h3>
          ...
          >
            {rows.length === 0 ? (
              <tr>
                <td colSpan={8} className="py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="font-medium text-text-primary">No terms found</p>
              ...
              rows.map((term, idx) => (
                ...
                >
                  <td className="px-4 py-4 text-text-muted font-medium">{start + idx + 1}</td>
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-2">
                      <BookOpen size={14} className="text-text-muted shrink-0" />
                      <span className={`font-semibold ${term.isActive ? "text-brand-primary" : "text-text-primary"}`}>
                        {term.name}
                      </span>
                    </div>
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex flex-col gap-1">
                      <span className="text-text-secondary text-xs">{term.sessionName}</span>
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 border border-blue-100 text-xs font-medium w-fit">
                        <span className="text-text-muted">#</span>{term.tag}
                      </span>
                    </div>
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-1.5 text-text-secondary">
                      ...
                      {dateFormat(term.startDate)}
                    </div>
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-1.5 text-text-secondary">
                      <CalendarDays size={13} className="text-text-muted shrink-0" />
                      {dateFormat(term.endDate)}
                    </div>
                  </td>
                  <td className="px-4 py-4">
...
function ResultBadge({ published }: { published: boolean }) {
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${
      published
