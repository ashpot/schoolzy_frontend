...
const SELECT_CLS =
  "appearance-none pl-3 pr-7 py-1.5 rounded-lg border border-[var(--color-border-line02)] bg-white text-xs text-[var(--color-text-nav)] focus:outline-none focus:ring-2 focus:ring-[var(--color-brand-primary)]/20 cursor-pointer";
...
export default function ScheduledTestsTable({ refreshKey }: Props) {
  ...
  return (
    <div className="bg-white rounded-2xl card-shadow overflow-hidden">
      {/* Header */}
      <div className="px-6 py-4 border-b border-[var(--color-border-line02)] flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-2">
          <h2 className="section-title">Manage Scheduled Tests</h2>
          <span className="w-6 h-6 rounded-full bg-blue-50 flex-center text-xs font-semibold text-[var(--color-brand-primary)]">
            {filtered.length}
          </span>
        </div>
          ...
          <thead>
            <tr className="border-b border-[var(--color-border-line02)]">
              {["#", "Test", "Class", "Date Scheduled", "Date Created", "Actions"].map((col) => (
                <th key={col} className={`py-3 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wide text-left ${col === "#" || col === "Actions" ? "px-6" : "px-4"}`}>
                  {col}
                </th>
              ))}
            </tr>
            ...
            {paginated.map((item, idx) => (
              ...
              >
                ...
                <td className="px-4 py-4">
                  <p className="text-sm font-medium text-[var(--color-text-nav)] mb-1.5">
                    {item.testTitle}
                  </p>
                  <div className="flex items-center gap-2">
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-1">
                    <button type="button" title="Edit" className="w-8 h-8 rounded-lg flex-center text-[var(--color-brand-primary)] hover:bg-blue-50 transition-colors">
      ...
      {filtered.length > 0 && (
        <div className="px-6 py-4 border-t border-[var(--color-border-line02)] flex items-center justify-between">
          <p className="text-sm text-[var(--color-text-muted)]">
            ...
            of{" "}
            <span className="font-medium text-[var(--color-text-nav)]">{filtered.length}</span>
          </p>
          <div className="flex items-center gap-1">
            <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} className="w-8 h-8 rounded-lg flex-center hover:bg-gray-100 disabled:opacity-30 transition-colors text-[var(--color-text-secondary)]">‹</button>
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <button key={p} onClick={() => setCurrentPage(p)} className={`w-8 h-8 rounded-lg flex-center text-sm font-medium transition-colors ${p === currentPage ? "bg-[var(--color-brand-primary)] text-white" : "text-[var(--color-text-secondary)] hover:bg-gray-100"}`}>{p}</button>
            ))}
            <button onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="w-8 h-8 rounded-lg flex-center hover:bg-gray-100 disabled:opacity-30 transition-colors text-[var(--color-text-secondary)]">›</button>
          </div>
        </div>
      )}
      ...
      {filtered.length === 0 && (
        <div className="py-14 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-sm font-semibold text-[var(--color-text-nav)]">No scheduled tests found</p>
