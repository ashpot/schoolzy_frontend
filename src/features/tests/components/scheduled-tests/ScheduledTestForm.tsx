    ...
    <div className="bg-white rounded-2xl card-shadow p-6">
      <div className="flex items-center gap-2 mb-6">
        <div className="w-6 h-6 rounded-md bg-blue-50 flex-center">
          <CalendarClock className="w-3.5 h-3.5 text-[var(--color-brand-primary)]" />
        </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--color-label)] mb-1.5">
              Date Scheduled <span className="text-[var(--color-danger)]">*</span>
            </label>
            <input
              type="date"
              disabled={isLoading}
              className={`w-full px-3 py-2.5 rounded-xl border bg-[var(--color-bg-input)] text-sm text-[var(--color-text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-brand-primary)]/20 focus:border-[var(--color-brand-primary)] transition-all disabled:opacity-60 ${
            ...
            {errors.dateScheduled && (
              <p className="mt-1 text-xs text-[var(--color-danger)]">{errors.dateScheduled.message}</p>
            )}
          </div>
        </div>
