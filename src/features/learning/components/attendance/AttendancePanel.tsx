...
import type { AttendanceStatus, AttendanceStudent } from "@/features/learning/types";
import { staggerContainer, rowVariant } from "@/features/learning/animations/variants";
...
import {
  ...
  STATUS_PILLS,
  FOOTER_STATS,
} from "@/features/learning/shared/utils/constants";
          ...
          <thead>
            <tr className="border-b border-[var(--color-border-line02)]">
              {[
                ...
                <th key={label} className={`${cls} text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wide`}>
                  {label}
                </th>
              ))}
            </tr>
            ...
            {paginatedStudents.map((student, idx) => (
              ...
              >
                ...
                <td className="px-4 py-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-full flex-center text-xs font-semibold flex-shrink-0 ${avatarColor(student.name)}`}>
                      {getInitials(student.name)}
                    </div>
                    <div>
                      <p className="text-sm font-medium text-[var(--color-text-nav)]">{student.name}</p>
                      <p className="text-xs text-[var(--color-text-muted)]">{student.admNo}</p>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-4">
      ...
      {filteredCount > 0 && (
        <div className="px-6 py-4 border-t border-[var(--color-border-line02)] flex items-center justify-between">
          <p className="text-sm text-[var(--color-text-muted)]">
            ...
            of{" "}
            <span className="font-medium text-[var(--color-text-nav)]">{filteredCount}</span> students
          </p>
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-[var(--color-text-muted)] max-w-xs">
        ...
        <span className="font-semibold text-[var(--color-text-nav)]">Load Students</span>
      </p>
    </motion.div>
  );
}